package com.ar.ui.tile
{
	 ...
	 * @version 0.1
	 */
	public final class TileMapRenderer implements ITileRenderer
	{
		...
		public function TileMapRenderer( output:BitmapData, tileSprites:BitmapData, tileMap:Array, spriteWidth:int, spriteHeight:int )
		{
			_output = output;
			...
			_outMaxRows = int( _output.height / _spriteWidth ) + 2;
			_outMaxCols = int( _output.width / _spriteHeight ) + 2;
		}

		public function render( offsetX:int, offsetY:int )
		{
			var mapColOffset:int = int( offsetX / _spriteWidth );
			var mapRowOffset:int = int( offsetY / _spriteHeight );

			var restX:int = offsetX % _spriteWidth;
			var restY:int = offsetY % _spriteHeight;
			...
			for( var i:int = 0; i < _outMaxRows; ++i )
			{
				...
				spriteOffset.y = ( i * _spriteHeight ) - restY;
